      ...
      <main className="mx-auto max-w-5xl px-5 py-10 sm:px-8 sm:py-14">
        ...
        {!loading && !error && course && <>
          ...
          <section aria-labelledby="lessons-heading" className="mt-10">
            ...
            {lessons.length === 0 ? <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center"><h3 className="font-semibold text-slate-900">No lessons yet</h3><p className="mt-2 text-slate-600">{ownsCourse ? "Create the first lesson for this course." : "The teacher has not added any lessons."}</p></div> : <ol className="mt-6 space-y-4">{lessons.map((lesson) => <li key={lesson.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex gap-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-50 text-sm font-bold text-indigo-700">{lesson.position}</span><div><h3 className="text-lg font-semibold text-slate-950">{lesson.title}</h3>{lesson.content ? <p className="mt-2 whitespace-pre-line leading-7 text-slate-600">{lesson.content}</p> : <p className="mt-2 text-sm text-slate-500">No lesson content provided.</p>}</div></div></li>)}</ol>}
          </section>
        </>}
      </main>
    </AppShell>
