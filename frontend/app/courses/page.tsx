    ...
    <AppShell>
      <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14 lg:px-10">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          {user?.role === "teacher" && <button type="button" onClick={() => setShowForm((value) => !value)} className="inline-flex shrink-0 items-center justify-center gap-2 self-start rounded-xl bg-indigo-600 px-4 py-3 font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 sm:self-auto"><span aria-hidden="true" className="text-lg leading-none">{showForm ? "×" : "+"}</span>{showForm ? "Close form" : "Create course"}</button>}
        </div>
        ...
        <section aria-label="Course list" className="mt-8">
          ...
          {!loading && !error && courses.length > 0 && <div className="grid items-stretch gap-5 sm:grid-cols-2 lg:grid-cols-3">{courses.map((course) => {
            const owned = user?.role === "teacher" && course.teacher_id === user.id;
            const enrolled = user?.role === "student" && enrolledCourseIds.has(course.id);
            return <article key={course.id} className="flex h-full min-h-80 flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-[border-color,box-shadow] hover:border-indigo-200 hover:shadow-md focus-within:border-indigo-300 focus-within:shadow-md"><div className="flex items-start justify-between gap-3"><span aria-hidden="true" className="flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-50 text-xl text-indigo-700">▤</span>{(owned || enrolled) && <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${owned ? "border-indigo-100 bg-indigo-50 text-indigo-700" : "border-emerald-100 bg-emerald-50 text-emerald-700"}`}>{owned ? "Your course" : "Enrolled"}</span>}</div><div className="mt-5 flex flex-1 flex-col"><h2 className="text-xl font-semibold leading-7 text-slate-950">{course.title}</h2><p className="mt-3 line-clamp-4 leading-7 text-slate-600">{course.description || "No description provided."}</p><p className="mt-auto pt-5 text-xs font-medium uppercase tracking-[0.12em] text-slate-500">Created {new Date(course.created_at).toLocaleDateString()}</p></div><Link href={`/courses/${course.id}`} className="mt-5 inline-flex items-center justify-between rounded-xl bg-indigo-50 px-4 py-3 text-sm font-semibold text-indigo-700 transition-colors hover:bg-indigo-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">View course <span aria-hidden="true" className="ml-3">→</span></Link></article>;
          })}</div>}
        </section>
      </main>
    </AppShell>
